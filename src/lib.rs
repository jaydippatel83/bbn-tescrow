use cosmwasm_std::{
    ...
    StdResult, Uint128, BankMsg, Coin, WasmMsg, SubMsg,
};
...
pub enum ExecuteMsg {
    ...
    Confirm {},
    StakeWithBabylon {
        _babylon_stake_token: String,  // The staked token address from Babylon
        amount: Uint128,
    },
}
...
pub fn execute_stake(deps: DepsMut, _env: Env, info: MessageInfo) -> StdResult<Response> {
    ...
    let required_stake = STAKE_AMOUNTS.load(deps.storage, &info.sender)?;
    let sent_amount: Uint128 = info
        .funds
        .iter()
        .find(|c| c.denom == "ujuno")
        .map(|c| c.amount)
        .unwrap_or_default();

    if sent_amount != required_stake {
        return Err(StdError::generic_err(format!(
            "Incorrect stake amount. Expected {}, got {}",
            required_stake, sent_amount
        )));
    }
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Stake {} => execute_stake(deps, _env, info),
        ExecuteMsg::RevokeStake {} => execute_revoke_stake(deps, _env, info),
        ExecuteMsg::Cancel {} => execute_cancel(deps, _env, info),
        ExecuteMsg::RevokeCancellation {} => execute_revoke_cancellation(deps, _env, info),
        ExecuteMsg::Confirm {} => execute_confirm(deps, _env, info),
        ExecuteMsg::StakeWithBabylon { _babylon_stake_token, amount } => {
            execute_stake_with_babylon(deps, _env, info, _babylon_stake_token, amount)
        },
    }
}
...
pub fn query_status(deps: Deps, _env: Env) -> StdResult<ContractStatus> {
    let state = STATE.load(deps.storage)?;
    ...
    Ok(ContractStatus {
        ...
        agreement_address: _env.contract.address,
    })
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::GetStatus {} => to_json_binary(&query_status(deps, _env)?),
    }
}
